...
def _codes(raw) -> list[int]:
    """"200, 401" → [200, 401]."""
    return [int(c) for c in str(raw or "").split(",") if c.strip().isdigit()]
...
def _dashy_item(entry: dict, default_status: bool, report: Report) -> dict | None:
    ...
    config = {
        ...
        "accept": _codes(entry.get("statusCheckAcceptCodes")),
...
def _dashy_widget(entry: dict, report: Report) -> dict | None:
    ...
    if target == "rss":
        config = {"url": options.get("rssUrl", ""), "limit": int(options.get("limit", 8))}
        return {"type": "rss", "title": title, "config": config}
    if target == "clock":
        zone = options.get("timeZone")
        config = {"timezones": [zone]} if zone else {}
        return {"type": "clock", "title": title, "config": config}
    if target == "weather":
    ...
    if target == "iframe":
        config = {"url": options.get("url", ""), "height": int(options.get("frameHeight", 320))}
        return {"type": "iframe", "title": title, "config": config}
