...
def grant(who: Principal, kind: ResourceKind, resource_id: int, grantee_kind: GranteeKind,
    ...
    with session_scope() as s:
        item = _resource(s, kind, resource_id)
        _need_manage(s, who, kind, item)
        if kind == ResourceKind.CONNECTION and ServiceType(item.service) == ServiceType.DAWARICH:
            if item.credential_mode == CredentialMode.SHARED and not misc.setting(s, "dawarich_shared").get("allowed"):
