...
def _section(s: Session, who: Principal, section: Section, layer: dict) -> SectionView:
    ...
    for placement in placements:
        ...
        if kind is None or _widget_right(who, s, widget) < Right.VIEW:
            continue
        view_.tiles.append(
            Tile(
...
def placed_widget(who: Principal, placement_id: int) -> Widget:
    with session_scope() as s:
        ...
        _load(s, who, placement.section.board_id, Right.VIEW)
        widget = placement.widget
        access.need(_widget_right(who, s, widget), Right.VIEW)
        return widget
